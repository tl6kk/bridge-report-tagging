...
def perpareDataset(embeddingsPath, datasets, frequencyThresholdUnknownTokens=5, reducePretrainedEmbeddings=False, valTransformations=None, padOneTokenSentence=True):
    """
    ...
    for datasetName, dataset in datasets.items():
        ...
        trainData = 'data/%s/train' % datasetName
        print(trainData)
        devData = 'data/%s/dev' % datasetName
        testData = 'data/%s/test' % datasetName
...
def readEmbeddings(embeddingsPath, datasetFiles, frequencyThresholdUnknownTokens, reducePretrainedEmbeddings):
    ...
    embeddingsIn = gzip.open(embeddingsPath, mode="rt", encoding="utf-8") if embeddingsPath.endswith('.gz') else open(embeddingsPath, encoding="utf-8")
    ...
    if frequencyThresholdUnknownTokens != None and frequencyThresholdUnknownTokens >= 0:
        ...
        for datasetName, datasetFile in datasetFiles.items():
            ...
            createFD(datasetPath + 'train', tokenIdx, fd, word2Idx)
        ...
        logging.info("Added words: " + ' '.join(addedWords))
